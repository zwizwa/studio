/* Stripped down C version of constrained parametric polymorphism
   ...
   references NS(_container_t), NS(_element_t)
*/
#ifndef NS
#error define NS
#endif
...
// Note that the return pointer is only valid until the next cycle around.
static inline NS(_element_t) *NS(_write)(NS(_container_t) *q, const NS(_element_t) *e) {
    NS(_element_t) *slot = NS(_write_hole)(q);
    if (slot) *slot = *e;
    return slot;
}
static inline NS(_element_t) *NS(_cycle)(NS(_container_t) *q) {
    const NS(_element_t) *slot = NS(_read_hole)(q);
    if (!slot) return NULL;
